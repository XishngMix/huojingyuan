...
def index2(page=1):
    ...
    if current_user.xy == 'admin':
        # pagination = Note_yet.query.paginate(
        ...
        return render_template('index.html', names=names, today=today, form=form)
    else:
        # pagination = current_user.articles.paginate(
        #     page, per_page=current_app.config['POST_PER_PAGE']
        # )
        # names = pagination.items
    ...
    # stu = Note_yet.query.get()
    # if current_user.xy == 'admin':
    #     pagination = Note_yet.query_all(
    #         name, starttime, endtime, matter, department, page
    #     )
    #     names = pagination.items
    #     return render_template('query.html', names=names, form=form, pagination=pagination)
    # else:
    #     pagination = Admin.query_all(
    #         name, starttime, endtime, matter, page
    #     )
    #     names = pagination.items
    #     return render_template('query.html', names=names, form=form, pagination=pagination)
...
# 查看全部办理人员信息
# @app.route("/all", methods=['GET'], defaults={'page': 1})
...
#
#         # return render_template('index.html', names=names,format_today=format_today)
#         return render_template('all.html', names=names, form=form, pagination=pagination, delete_form=delete_form)
#     else:
#         # names = current_user.articles
#         pagination = current_user.articles.paginate(
#             page, per_page=current_app.config['POST_PER_PAGE']
...
#         # school_endtime = form.school_endtime.data           #离校时间
...
#         if userxh and student.xm == name:  # 有业务逻辑错误
#             if (student.status == 'tx' or student.status == 'xx') and matter == 'xj':
#                 return '处于休学或退学状态，无法申请在校生学籍'
#             else:
#                 student_info = Note_yet(xh=userxh.xh, xm=userxh.xm, xy=userxh.xy, bj=userxh.bj, zt=matter,
#                                         admin_id=userxh.admin_id, reason=reason)
#                 db.session.add(student_info)
#                 db.session.commit()
#                 # flash("成功申请，请找老师打印")
#                 return '提交成功，请通知老师在系统进行下一步工作'
#         # else:
...
#
#     return render_template('sun.html', form=form)
...
# # 查询
# @app.route("/query", methods=['GET'], defaults={'page': 1})
# @app.route('/page/<int:page>')
# @login_required
# def query(page):
#     form = QueryForm()
...
#         )
#         names = pagination.items
#         return render_template('query.html', names=names, form=form, pagination=pagination, delete_form=delete_form)
#     else:
#         # if endtime == '':
#         #     endtime = starttime
#         # pagination = current_user.articles.filter(
#         #     (Note_yet.xm == name) if name is not None else "",
#         #     Note_yet.created_date.between(starttime, endtime) if starttime is not None else "",
#         #     Note_yet.zt == matter if matter != 'info' else "").paginate(
#         #     page, per_page=current_app.config['POST_PER_PAGE']
#         # )
#         # names = pagination.items
#         # return render_template('query.html', names=names, form=form, pagination=pagination)
...
# 教师编辑
# @app.route('/edit/<int:name_id>', methods=['POST', 'GET'])
# @login_required
...
#     form.created_date.data = name.created_date
...
def edit_student(name_id):
    ...
    # if name.discipline == 'none' and name.campus == 'none':
    #     form.campus = name.xy
    #     form.discipline = name.bj
    # else:
    #     form.discipline = name.discipline
    #     form.campus = name.campus
...
def new_all(page):
    ...
    if current_user.xy == 'admin':
        ...
        # return render_template('index.html', names=names,format_today=format_today)
        return render_template('new_all.html', names=names, form=form, pagination=pagination, delete_form=delete_form)
    else:
        # names = current_user.articles
        pagination = current_user.articles.paginate(
            page, per_page=current_app.config['POST_PER_PAGE']
